...
cobb.dataFrame <- as.data.frame(cobb.mergecase)
cobb.dataFrame%>%
  group_by(ZIP_CODE,yearDate)%>%
  arrange(yearDate)
nrow(cobb.dataFrame)
...
ggplot(cobb.dataFrame, aes(POPULATION, case, size = POPULATION, colour = ZIP_CODE)) +
  geom_point(alpha = 0.7, show.legend = TRUE) +
  geom_text(aes(label=ZIP_CODE),color="dodgerblue",nudge_x=3,hjust=0,size=4)+
